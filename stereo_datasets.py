...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/zjz/data/sceneflow/', dstype='frames_finalpass', things_test=False):
...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/zjz/data/ETH3D', split='training'):
        super(ETH3D, self).__init__(aug_params, sparse=True)
        assert os.path.exists(root)

        image1_list = sorted( glob(osp.join(root, f'two_view_{split}/*/im0.png')) )
        image2_list = sorted( glob(osp.join(root, f'two_view_{split}/*/im1.png')) )
        disp_list = sorted( glob(osp.join(root, f'two_view_{split}/*/disp0GT.pfm')) ) if split == 'training' else [osp.join(root, 'two_view_training_gt/playground_1l/disp0GT.pfm')] * len(image1_list)
...
class CREStereoDataset(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/zjz/data/CREStereo'):
...
class InStereo2K(StereoDataset):
    def __init__(self, aug_params=None, root='/root/autodl-tmp/zjz/data/InStereo2K'):
        super(InStereo2K, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispInStereo2K)
        assert os.path.exists(root)

        image1_list = sorted(glob(root + '/train/*/*/left.png') + glob(root + '/test/test/*/left.png'))
        image2_list = sorted(glob(root + '/train/*/*/right.png') + glob(root + '/test/test/*/right.png'))
        disp_list = sorted(glob(root + '/train/*/*/left_disp.png') + glob(root + '/test/test/*/left_disp.png'))
